...
class Heist(commands.Cog):
    ...
    async def _play_heist(self, ctx):
        ...
        if not config["Planned"]:
            ...
            await ctx.send("\n <@&824570460237987870> A {4} is being planned by {0} \n The {4}"
                               "will begin in {1} seconds. Type {2}heist play to join their" 
                               "{3}.".format(author.name, wait_time, ctx.prefix, t_crew, t_heist) allowed_mentions=discord.AllowedMentions(roles=True))
